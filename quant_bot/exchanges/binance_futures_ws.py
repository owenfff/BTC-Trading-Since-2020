...
from .binance_futures_http import BINANCE_FUTURES_TESTNET_WS_URL, BinanceFuturesTestnetTransport, assert_binance_futures_testnet_ws_url
from .http import AdapterError


class BinanceFuturesTestnetWebSocket:
    """USDⓈ-M Futures Testnet user-data stream backed by a REST listen key."""

    def __init__(self, transport: BinanceFuturesTestnetTransport, *, url: str = BINANCE_FUTURES_TESTNET_WS_URL, connect_factory: Any | None = None) -> None:
        assert_binance_futures_testnet_ws_url(url)
    ...
    def _create_listen_key(self) -> str:
        response = self.transport.request_api_key("POST", "/fapi/v1/listenKey")
        if not isinstance(response, dict) or not response.get("listenKey"):
            raise AdapterError("binance-futures-testnet", "SCHEMA", "Binance Futures user-data stream response missing listenKey")
    ...
    async def receive(self) -> dict[str, Any]:
        if self.socket is None:
            raise AdapterError("binance-futures-testnet", "WEBSOCKET_NOT_CONNECTED", "connect before receive")
        raw = await self.socket.recv()
        message = json.loads(raw)
        if isinstance(message, dict):
            self.accept_message(message)
            return message
        raise AdapterError("binance-futures-testnet", "SCHEMA", "Binance Futures user-data stream returned a non-object message")
...
__all__ = ["BinanceFuturesTestnetWebSocket"]
