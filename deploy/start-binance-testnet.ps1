...
$oldKey = [Environment]::GetEnvironmentVariable("BINANCE_TESTNET_API_KEY", "Process")
$oldSecret = [Environment]::GetEnvironmentVariable("BINANCE_TESTNET_API_SECRET", "Process")
$credentialStorePath = Join-Path $repo "quantoutputsinance_testnet_credentials.json"
$exitCode = 1
...
function Assert-AsciiCredential([string]$Name, [string]$Value) {
    ...
    foreach ($character in $Value.ToCharArray()) { if ([int][char]$character -gt 127) { throw "$Name contains non-ASCII characters. Use the raw credential." } }
}
function Read-DpapiCredentials([string]$Path) {
    ...
    } catch { throw "Local Binance DPAPI credential store could not be decrypted for this Windows user. Use -ForgetCredentials to replace it." }
}
...
try {
    if (-not $PythonPath) {
        $candidates = @((Join-Path $env:USERPROFILE ".cachecodex-runtimescodex-primary-runtimedependenciespythonpython.exe"), (Join-Path $env:LOCALAPPDATA "ProgramsPythonPython311\python.exe"), (Join-Path $env:LOCALAPPDATA "Programs\Python\Python313\python.exe"))
    ...
    if (-not $env:BINANCE_TESTNET_API_KEY -and $stored) { $env:BINANCE_TESTNET_API_KEY = $stored.ApiKey }
    if (-not $env:BINANCE_TESTNET_API_SECRET -and $stored) { $env:BINANCE_TESTNET_API_SECRET = $stored.ApiSecret }
    ...
    if (-not $stored -or $ForgetCredentials) { Save-DpapiCredentials $credentialStorePath $env:BINANCE_TESTNET_API_KEY $env:BINANCE_TESTNET_API_SECRET }
    $env:PYTHONPATH = "$repo;$repo\quant\src"
    & $PythonPath -m quant_bot preflight --venue binance-spot-testnet
    if ($LASTEXITCODE -ne 0) { throw "Binance Spot Testnet preflight failed with exit code $LASTEXITCODE" }
    $runArgs = @("-m", "quant_bot", "run", "--venue", "binance-spot-testnet", "--mode", "testnet", "--symbols", "auto", "--poll-seconds", "60", "--allow-spot-approximation")
    ...
    if ($null -eq $oldKey) { Remove-Item Env:BINANCE_TESTNET_API_KEY -ErrorAction SilentlyContinue } else { $env:BINANCE_TESTNET_API_KEY = $oldKey }
    if ($null -eq $oldSecret) { Remove-Item Env:BINANCE_TESTNET_API_SECRET -ErrorAction SilentlyContinue } else { $env:BINANCE_TESTNET_API_SECRET = $oldSecret }
}
exit $exitCode
